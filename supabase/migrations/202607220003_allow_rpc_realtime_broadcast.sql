-- Permite que broadcast_coup_room_event grave em realtime.messages sem dar ao
-- WebSocket do navegador permissão para publicar Broadcast diretamente.
...
drop policy if exists "room members can publish private presence" on realtime.messages;
drop policy if exists "room members can publish presence or rpc broadcast" on realtime.messages;
