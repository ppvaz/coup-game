...
export function lobbyHTML(state) {
  const botCount = normalizeLocalBotCount(state.botCount);
  const botCountField =
    state.mode === 'bots'
      ? `<div class="field bot-count"><label id="bot-count-label">Quantos rivais?</label><div class="bot-count-grid" role="group" aria-labelledby="bot-count-label">${LOCAL_BOT_COUNTS.map((count) => `<button type="button" class="bot-count-option ${count === botCount ? 'active' : ''}" data-bot-count="${count}" aria-pressed="${count === botCount}">${count}</button>`).join('')}</div><small>${botCount} ${botCount === 1 ? 'bot' : 'bots'} + você · ${botCount + 1} jogadores</small></div>`
      : '';
  ...
  const scenarioField =
    state.mode === 'bots'
      ? `<div class="field bot-count"><label id="scenario-label">Cenário</label><div class="bot-count-grid" role="group" aria-labelledby="scenario-label">${Object.values(
          ...
          .map(
            (option) =>
              `<button type="button" class="bot-count-option ${option.id === scenario ? 'active' : ''}" data-scenario="${option.id}" aria-pressed="${option.id === scenario}">${option.label}</button>`,
          )
